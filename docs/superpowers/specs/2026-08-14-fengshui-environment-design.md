...
采用三元命卦通行算法。以立春年 `Y` 直接取模，**无需分 1900s / 2000s 两套式子**：
...
余 0 归 9；结果为 5（中宫无卦）时，男寄坤(2)、女寄艮(8)。

等价于坊间通行的分段写法（男「(100 − 后两位) ÷ 9 取余」、女「(后两位 − 4) ÷ 9 取余」），但世纪分支被消掉了——因 `1900 mod 9 = 1`、`2000 mod 9 = 2`，两段折算后同式。

> **已对拍（2026-08-14）**：公式与六个基准值核对自公开命卦速查表（华易网 `k366.com/minggua/`、知乎命卦对照表）——1984 男兑7 / 女艮8、1990 男坎1 / 女艮8、1991 男离9 / 女乾6。同来源确认立春分界（「二月四日或五日之前出生按旧的一年计算」）。命卦算法存在流派变体，本项目**选定上述三元通行式**，与 iztro `algorithm: 'zhongzhou'` 的显式选派做法一致，并由测试锁定。
...
依据**大游年歌**（以坐山为伏位，其余七字**按方位顺时针**依次排；简写 生=生气 天=天医 延=延年 五=五鬼 六=六煞 祸=祸害 绝=绝命）：

> 乾六天五祸绝延生 · 坎五天生延绝祸六 · 艮六绝祸生延天五 · 震延生祸绝五天六
> 巽天五六祸生绝延 · 离六五绝延祸生天 · 坤天延绝生祸五六 · 兑生祸延绝六五天

展开后（**已逐格对拍，2026-08-14**）：
...
> ⚠️ **六煞与祸害同为四凶、且同落一组方位**，因此「四吉方全落本组方位」这类结构性测试**抓不到二者互换**。初稿中艮、震、离、坤四行正是在此处出错，只能靠逐格断言守护。
