...
## 🟡 MED
- [ ] [EP-cal-img] [MED] 运势日历配图：为当日运势寻找/生成/显示一张匹配的图片（趋吉避祸主题/五行/季节）。注意**风格匹配**照见基调（宣纸/宋体/朱墨，水墨写意，非写实/非彩色卡通）。来源可选：① 预置主题图库按规则匹配 ② 生成式（需评估成本/风格一致性）。
- [ ] [EP-profile-q] [MED] 建档增加交互式心理学问卷：起盘流程中插入若干心理学问题（如自我认知/关系/动机倾向），结果并入 LLM 解读上下文以完善对用户的分析理解（与命盘事实互证，但需标注主观自陈 vs 命盘客观）。设计上注意不增加过多起盘摩擦（可「先出盘、后渐进追问」）。
