...
Last Updated:   2026-08-21 by claude-sonnet-5（EP-002-cal-2 调候用神）
线上:           https://zhaojian-mvp.vercel.app · zhaojian.agentjoey.ai
测试:           core 162 · llm 262 · web 470（全绿；`lint` 0 errors 已为阻塞闸门）
⚠️ `pnpm typecheck`：core/llm 全绿，**apps/web 有 7 处既存类型错误**（`account`/`dream`/`auth/callback`/`merge-anon` 各测试文件的 mock 类型，`EP-account-login` 47bd1b1 引入，2026-08-20，与今日改动无关）——此前各交付记录写「typecheck 0」实为只单独跑过对应包、未跑过本仓库顶层 `pnpm run typecheck` 复核；已发现即记录，见 BACKLOG `EP-web-typecheck-debt`。
